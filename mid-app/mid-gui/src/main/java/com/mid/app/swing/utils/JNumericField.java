...
public class JNumericField extends JTextField {
	...
	public JNumericField(final int iMaxLen) {
		this(iMaxLen, NUMERIC);
	}

	public JNumericField(final int iMaxLen, final int iFormat) {
	...
	public void setPrecision(final int iPrecision) {
		if (format == NUMERIC)
			return;
	...
	class JNumberFieldFilter extends PlainDocument {
		...
		public void insertString(final int offset, final String str, final AttributeSet attr)
			...
			final String text = getText(0, offset) + str
			...
			try {
				if (format == NUMERIC) {
					...
					final int dotIndex = text.indexOf(DOT);
					if (dotIndex != -1) {
						dotLength = 1;
